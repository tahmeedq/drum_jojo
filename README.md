...
- **Six swappable kits** — Acoustic, Stark, Roland R8, Techno, LinnDrum and
  CR-78, bundled offline in `public/samples/` with a kit picker in the header.
  Each ships kick/snare/hi-hat/3 toms.
- **Sampled cymbals** — crash, ride and open hi-hat use CC0 cymbal samples
  (Versilian VCSL) in `public/samples/_cymbals/`, layered on top of every kit;
  an inharmonic metal-oscillator synth is the fallback. Cross-stick stays synth.
- **Big content library** — 70+ grooves (rock, pop, funk, neo-soul, hip-hop/trap,
  metal/double-bass, blues/shuffle, jazz, latin/world, reggae, drum & bass,
  gospel/fusion, odd time), 50+ fills (incl. a full double-bass family), 29
  rudiments, 9 play-along songs, and a 35-lesson / 10-level course.
...
Kit samples are from the [Tone.js audio](https://github.com/Tonejs/audio)
library (MIT). Cymbal samples (crash/ride/open hi-hat) are from the
[Versilian Community Sample Library](https://github.com/sgossner/VCSL) (CC0),
transcoded to AAC. Both live under `public/samples/`.
