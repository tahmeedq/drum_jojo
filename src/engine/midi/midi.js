/* ============================================================
   MIDI-in timing + dynamics engine for electronic kits.
   ...
   note of the same voice, and grade:
     • timing   — offset in ms (− = rushing, + = dragging)
     • dynamics — did the played velocity match the notated accent /
                  normal / ghost dynamic?
   ============================================================ */
...
let expected = [];            // {id, time, step, v, used}
const recent = [];           // last N signed ms offsets
let totalMatched = 0, totalInGood = 0, totalDynOk = 0;

// Collect scheduled notes to grade against.
on("expectHit", e => expected.push({ id: e.id, time: e.time, step: e.step, v: e.v, used: false }));
...
// Is the played MIDI velocity (0..127) consistent with the notated dynamic?
//   v===2 accent → loud   v===3 ghost → soft   else normal → mid
function dynamicsOk(v, midiVel) {
  if (v === 2) return midiVel >= 80;
  if (v === 3) return midiVel <= 55;
  return midiVel > 30;
}
...
export const MIDI = {
  ...
  _grade(id, tC, midiVel) {
    ...
    emit("midiHit", { id, step: best.step, deltaMs, rating, dynOk, expectV: best.v, matched: true });
    this._emitStats();
  },
  ...
  _emitStats() {
    ...
    const dynAcc = totalMatched ? Math.round(totalDynOk / totalMatched * 100) : 0;
    emit("timingStats", { mean, spread: Math.sqrt(variance), acc, dynAcc, samples: totalMatched });
  },
